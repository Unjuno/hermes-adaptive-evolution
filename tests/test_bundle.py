...
def test_bundle_roundtrip_matches_direct_replay(tmp_path: Path, monkeypatch):
    ...
    assert created["manifest"]["schema"] == "adaptive-evolution.capture-bundle.v0.1"
    ...
    trace = (target / "normalized-events.jsonl").read_text(encoding="utf-8")
    assert "secret-value" not in trace
    assert "sensitive output" not in trace
    assert str(tmp_path) not in (target / "manifest.json").read_text(encoding="utf-8")


def test_bundle_checksum_detects_mutation(tmp_path: Path):
    db = _fixture_db(tmp_path)
    target = tmp_path / "bundle"
...
def test_bundle_manifest_is_machine_readable(tmp_path: Path):
    ...
    assert manifest["trace_file"] == "normalized-events.jsonl"
